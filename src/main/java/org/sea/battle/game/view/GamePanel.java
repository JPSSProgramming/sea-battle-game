...
public class GamePanel extends JPanel implements Stoppable {
    ...
    public GamePanel(GameLogic logic, boolean vsAI) {
        this(logic, vsAI, null);
    }

    public GamePanel(GameLogic logic, boolean vsAI, Consumer<Player> onGameOver) {
        this.logic = logic;
        this.vsAI = vsAI;
        ...
        SwingUtilities.invokeLater(this::playAITurnsIfNeeded);
    }
    ...
    private void handleHover(GameBoard src, MouseEvent e) {
        if (finished || logic.getCurrentPlayer() instanceof AI) return;
    ...
    private void handleClick(GameBoard clicked, MouseEvent e) {
        ...
        playFeedback(outcome, opponentBoard, x, y);
    ...
    private void playFeedback(GameLogic.ShotOutcome outcome, GameBoard targetBoard, int x, int y) {
        if (outcome.sunkShip() != null) {
            animateSunk(outcome.sunkShip(), targetBoard);
            targetBoard.getParticles().burst(
    ...
    private void playAITurnsIfNeeded() {
        while (!finished && logic.getCurrentPlayer() instanceof AI ai) {
            if (logic.isSalvoMode()) {
    ...
    private GameLogic.ShotOutcome fireAIShot(AI ai) {
        ...
        refreshFleetTrackers();
        updateStatus(logic.getCurrentPlayer().getName(), outcome);
        playFeedback(outcome, targetBoard, t[0], t[1]);
    ...
    private void endWithWinner(Player winner) {
        ...
        boolean humanWon = !(winner instanceof AI);
        if (humanWon) {
    ...
    public Difficulty findAiDifficulty() {
